# Auditor Bancario VCT — Resumen público (versión gratuita)

**Documento:** v1.0-gratis · **Proyecto:** Vanilla Center Trust [VCT]  
**Autor:** Ángel (calero89) · © 2026 · **Estado:** desarrollo constante

---

## En una frase

Bot de Discord que gestiona la **economía, reputación y trámites** de VCT: dinero legal, caja fuerte, dinero negro, nóminas por rangos, trabajos FS22, juegos, mercado negro, tiendas interactivas, alarma/persecución/arresto y automatizaciones 24/7.

**Última versión pública documentada:** **2.9.4** (parches de seguridad economía — Jun 2026).

---

## ¿Qué puedes hacer hoy?

- Consultar tu **cuenta** (`/bal`), **reputación** y **cumpleaños** con bonus anual.  
- **Depositar**, **retirar** y **transferir** dinero; **cobrar** sueldo según tu rango en Discord.  
- Vincular **Farming Simulator 22**, ver perfil e historial; el staff registra **contratos** con recibo.  
- **Robar**, **lavar** dinero negro, **mercado negro** e **inventario** (mecánica de rol).  
- **Juegos**: trabajo rápido, submundo, trabajos comunitarios, rasca, dados, tragaperras, ruleta.  
- Panel **/ticket**: crear contrato o pedir apelación.  
- **Tiendas fijadas** en Discord (legal, especial, socio) + **alarma de seguridad**, **/perseguir** y **arresto** con fianza (v2.9.x).

Al entrar al servidor recibes el rol **✅┃Ciudadano de VCT** y se genera tu **NRB** (número de registro bancario).

---

## Economía (lo esencial)

| | |
|---|---|
| **Cartera** | Dinero legal |
| **Caja fuerte** | Ahorro (impuesto al mover) |
| **Dinero negro** | Hay que lavarlo; caduca si no lo haces |
| **Fondo central** | Impuestos del servidor (5%, 15% lavado, 20% semanal) |

Reputación **trabajador** y **criminal** según tus acciones.

---

## Hacia dónde vamos (con tu apoyo)

1. **Hoy** — Bot activo en Discord + FS22 + servidor dedicado 24/7.  
2. **Próximo** — **Hosting propio** y **servidor de juego exclusivo VCT**.  
3. **Futuro** — **Bot + juego unidos**: registrar movimientos de cada jugador en el servidor y que el bot **pague o sancione en Discord** según lo que ocurra en partida.

El informe **completo** (comandos detallados, roadmap y visión técnica) está en el tier de mecenas.

---

## ¿Por qué Patreon?

Mantener hosting, desarrollo y la integración juego↔Discord requiere tiempo y costes. Los mecenas ayudan a que VCT tenga economía seria, divertida y conectada con el campo.

**Gracias por formar parte de Vanilla Center Trust.**

— Ángel (calero89) · Auditor Bancario VCT

*Versión gratuita v1.0 — sin datos técnicos ni secretos. Informe ampliado reservado a mecenas.*
